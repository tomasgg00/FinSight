...
def get_market_summary():
    try:
        con = duckdb.connect(DB_PATH, read_only=True)
        df = con.execute("""
            SELECT summary_date, generated_at, summary
...
if page == "Market overview":
    ...
    if not summary_df.empty:
        ...
        st.markdown(f"""
        <div style="background:#1A1D27;border:1px solid #2A2D3A;border-left:3px solid #1D9E75;
                    border-radius:12px;padding:1.2rem 1.5rem;margin-bottom:1.5rem;">
            <div style="font-size:11px;color:#888;text-transform:uppercase;
                        letter-spacing:0.08em;margin-bottom:8px;">
                AI Market Brief — {row['summary_date']}
            </div>
            <div style="font-size:14px;color:#E8E8E8;line-height:1.7;">
                {row['summary']}
            </div>
            <div style="font-size:11px;color:#888;margin-top:8px;">
                Generated by Mistral at {pd.to_datetime(row['generated_at']).strftime('%H:%M UTC')}
            </div>
        </div>
        """, unsafe_allow_html=True)
